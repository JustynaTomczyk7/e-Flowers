...
export function BasketLayout() {
  const { products, removeProductFromBasket, incrementCount, decrementCount } =
          ...
          {products.map(({ id, img, name, price, count }) => (
            <div className="added-product" key={id}>
              <Link className="link" to={`/${id}`}>
                <img className="b-img" src={img} alt="Bukiet Kwiatów" />
              </Link>
              <Link className="link" to={`/${id}`}>
              ...
              <div className="b-quantity">
                <p
                  className="b-quantity-plus-minus"
